...
export default function AboutContent({ about }: { about?: SanityAbout | null }) {
  const imageSrc = about?.profileUrl || FALLBACK_IMAGE;
  const bio = about?.bio && about.bio.length > 0 ? about.bio : fallbackBio;
  const stats =
    about?.stats && about.stats.length > 0 ? about.stats : fallbackStats;
  const journey =
    about?.journey && about.journey.length > 0
      ? about.journey
      : fallbackJourney;
  const styleDescription = about?.styleDescription || fallbackStyle;
  const genres =
    about?.genres && about.genres.length > 0 ? about.genres : fallbackGenres;
    ...
    <main className="min-h-screen pt-24 pb-16">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 pt-8">
                {stats.map((stat, index) => (
                  <div
                    ...
                    className="bg-white/[0.03] border border-white/5 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] text-center flex flex-row sm:flex-col items-center justify-center gap-2 sm:gap-0"
                  >
                    <div
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
            {journey.map((milestone, index) => (
              ...
              >
                <div className="text-3xl md:text-4xl font-black text-white/10 absolute bottom-4 right-4 md:bottom-6 md:right-8 group-hover:text-white/20 transition-colors">
                  {milestone.year}
                </div>
                <h3 className="text-base md:text-lg font-bold text-white mb-4 uppercase tracking-widest">
                  {milestone.title}
                </h3>
      ...
      <section className="py-24">
        <div className="container mx-auto px-6">
          ...
          >
            <h2 className="text-xl sm:text-3xl md:text-4xl font-bold mb-8 uppercase tracking-[0.05em] sm:tracking-[0.1em] flex flex-col sm:flex-row justify-center items-center gap-1 sm:gap-0">
              <span className="gradient-text">MUSICAL</span>
              <span className="text-white sm:ml-4">STYLE</span>
            </h2>
